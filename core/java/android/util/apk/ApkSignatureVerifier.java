    ...
    private static ParseResult<SigningDetailsWithDigests> verifyV1Signature(ParseInput input,
        ...
        try {
            ...
            if (sPerfBoost != null && !sIsPerfLockAcquired && verifyFull) {
                ...
                Slog.d(LOG_TAG, "Perflock acquired for PackageInstall ");
                        ...
                        public void run() {
                            try {
                                if (vData.exceptionFlag != 0 ) {
                                    Slog.w(LOG_TAG, "VerifyV1 exit with exception " + vData.exceptionFlag);
                                    return;
                                }
                ...
                while (vData.wait) {
                    try {
                        if (vData.exceptionFlag != 0 && !vData.shutDown) {
                            Slog.w(LOG_TAG, "verifyV1 Exception " + vData.exceptionFlag);
                        ...
                        Slog.w(LOG_TAG,"VerifyV1 interrupted while awaiting all threads done...");
                    }
                }
                if (vData.exceptionFlag != 0)
            ...
            if (sIsPerfLockAcquired && sPerfBoost != null) {
                ...
                Slog.d(LOG_TAG, "Perflock released for PackageInstall ");
            }
