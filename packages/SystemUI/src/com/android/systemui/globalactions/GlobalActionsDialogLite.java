...
public class GlobalActionsDialogLite implements DialogInterface.OnDismissListener,
    ...
    private static final String SETTING_ADVANCED_REBOOT = "advanced_reboot";
    ...
    private boolean shouldShowRestartSubmenu() {
        ...
        if (userId != UserHandle.USER_SYSTEM
                || mSecureSettings.getIntForUser(
                        SETTING_ADVANCED_REBOOT, 0, userId) != 1) {
            return false;
        }

        return !(mKeyguardShowing && mKeyguardStateController.isMethodSecure());
    }

    @VisibleForTesting
    String[] getRestartActions() {
        return mResources.getStringArray(
                com.android.systemui.res.R.array.config_restartActionsList);
    }
    ...
    private abstract class RestartReasonAction extends SinglePressAction {
        RestartReasonAction(int icon, int label) {
            super(icon, label);
        }
        ...
        @Override
        public void onPress() {
            mWindowManagerFuncs.reboot(false, getReason());
        }
    }
    ...
    private final class RestartDownloadAction extends RestartReasonAction {
        ...
        @Override
        String getReason() {
            return PowerManager.REBOOT_DOWNLOAD;
        }
    }
